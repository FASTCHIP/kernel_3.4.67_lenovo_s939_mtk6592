...
#if 1 //modify by lenovo.sw2 houdz1 2014.02.24
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led) {
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			upmu_set_isink_ch0_step(0x0);//16mA //4ma lenovo.sw2 houdz 20140305
		...
		case MT65XX_LED_PMIC_NLED_ISINK1:
			...
			upmu_set_isink_ch1_step(0x0);//16mA //4ma lenovo.sw2 houdz 20140305
	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK)
	{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
		{
			...
			if (level) 
			{
				...
				/* Lenovo-sw yexm1 add for use LP5560, 20140401 begin */
				#if defined (LENOVO_LP5560_SUPPORT)
				mt_set_gpio_mode(GPIO20, GPIO_MODE_00);
				mt_set_gpio_dir(GPIO20, GPIO_DIR_OUT);
				mt_set_gpio_out(GPIO20, GPIO_OUT_ONE);
				#endif
				/* Lenovo-sw yexm1 add for use LP5560, 20140401 end */
			}
			else 
			{
				upmu_set_isink_ch1_en(0x0);
