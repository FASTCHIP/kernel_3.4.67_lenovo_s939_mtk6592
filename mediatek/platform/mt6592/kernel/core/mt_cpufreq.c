...
static unsigned int mt_cpufreq_limit_max_freq_by_early_suspend(void)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_cpufreq_limit_max_freq_early_suspend policy->max=%d\n", policy->max);
    if (policy->max < DVFS_F4)
    {
...
#else// LENOVO_CPUFREQ_LIMIT
    cpufreq_driver_target(policy, DVFS_F4, CPUFREQ_RELATION_L);
    xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_cpufreq limited max freq by early suspend %d\n", DVFS_F4);
#endif//LENOVO_CPUFREQ_LIMIT
    /* lenovo.sw end chenyb1 add for cpufreq limit in earlysuspend */
...
/* lenovo.sw begin.chenyb1 20131120.close micro thermal, it is just for 2.0G cpu */
#if 1
static int mt_cpufreq_ptpod_temperature_limit_1 = 90000; //65000; //65C
static int mt_cpufreq_ptpod_temperature_limit_2 = 99000; //90000; //90C
#else
...
#endif
/* lenovo.sw end.chenyb1 20131120.close micro thermal, it is just for 2.0G cpu */
...
static int mt_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    *************************************************/
    if(mt_cpufreq_limit_max_freq_early_suspend == true)
    {
        ...
        {
            freqs.new = policy->max;
            xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "mt_cpufreq_limit_max_freq_early_suspend freqs.new=%d,policy->max=%d\n", freqs.new,  policy->max);
        }
