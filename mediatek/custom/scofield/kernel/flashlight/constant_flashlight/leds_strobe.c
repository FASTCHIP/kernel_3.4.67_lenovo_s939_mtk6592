...
int FL_Enable(void)
{
	...
	BD7710_write_reg(BD7710_i2c_client, 0x05, reg_val | 0x02);
	if(g_duty == 0)//torch
	{
		BD7710_write_reg(BD7710_i2c_client, 0x02, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x01, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x00, 0x06); //150ma torch output_en'
		udelay(50);
				  ...
				  enum led_brightness brightness)
{
	...
	else
	{
		...
		BD7710_write_reg(BD7710_i2c_client, 0x02, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x01, 0x40);
		BD7710_write_reg(BD7710_i2c_client, 0x00, 0x06); //only led2 150

		//BD7710_write_reg(BD7710_i2c_client, 0x00, 0x05);
		udelay(50);
		mt_set_gpio_out(GPIO_ENF, GPIO_OUT_ONE);
	}
	#elif defined(FLASH_RT9387)
