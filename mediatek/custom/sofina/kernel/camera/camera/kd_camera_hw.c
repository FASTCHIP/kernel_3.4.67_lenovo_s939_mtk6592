...
#define IDX_PS_OFF  3
	...
	if (On) {
		...
		if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825_MIPI_RAW,currSensorName))) {
			/*Lenovo-sw chenglong1 add for  ov8825*/
			if(pinSetIdx == 1) {
				PK_DBG("[CAMERA SENSOR] CAM_ERR pinsetIdx=%d, name: %s\n", pinSetIdx, currSensorName);
				return 0;
			}
		...
		{
			mdelay(10);
			/*Lenovo-sw chenglong1 add for front level stablity*/
			if(mt_set_gpio_pull_enable(pinSet[pinSetIdx][IDX_PS_CMPDN], 1)) {PK_DBG("[CAMERA LENS] set gpio enable pull failed!! \n");}
			if(mt_set_gpio_pull_select(pinSet[pinSetIdx][IDX_PS_CMPDN], GPIO_PULL_UP)) {PK_DBG("[CAMERA LENS] set gpio pull up failed!! \n");}
			/*Lenovo-sw add end*/
			if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
			mdelay(5);
			if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
		...
		if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825_MIPI_RAW,currSensorName)))
		{
			...
			//specical config for sp0a89, the power sequence is unnormally
			/*Lenovo-sw chenglong1 add for sub senosr*/
			if(pinSetIdx ==0) {
				PK_DBG("[CAMERA SENSOR] CAM_ERR pinsetIdx=%d, name: %s\n", pinSetIdx, currSensorName);
				return 0;
			}
