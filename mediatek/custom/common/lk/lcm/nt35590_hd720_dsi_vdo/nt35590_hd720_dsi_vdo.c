...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		/* Lenovo-sw yexm1 modify, 20140425 */
		params->dsi.horizontal_sync_active				= 2;//2
		params->dsi.horizontal_backporch				= 34;//34
		params->dsi.horizontal_frontporch				= 34;//24
		params->dsi.horizontal_active_pixel				= FRAME_WIDTH;
#endif
		/* Lenovo-sw yexm1 add, 20131217 begin */
		params->bl_app.min =1;
		params->bl_app.def =102;
		params->bl_app.max =255;

		params->bl_bsp.min =6;
		params->bl_bsp.def =94;
		params->bl_bsp.max =255;
		/* Lenovo-sw yexm1 add, 20131217 end */
		...
		/*Lenovo-sw yexm1 modify, 20131115 */
...
#if defined(BUILD_LK)
	printf("%s, [yexm1]nt35590 GPIO92 = %d \n", __func__, ret);
#endif	
...
//CMD1
	...
	dsi_set_cmdq(array, 2, 1);
  	...
  	read_reg_v2(0xF4, buffer,1);//0xF4
  	id = buffer[0]; 

 
  	#ifdef BUILD_LK
   	 printf("[LK]LCM %s nt35590 id = %x \n",__func__,id);
  	#else
    	 printk("[KERNEL] LCM %s nt35590 id = %x\n",__func__,id);
  	#endif
  	return (0x90 == id)?1:0;
}
