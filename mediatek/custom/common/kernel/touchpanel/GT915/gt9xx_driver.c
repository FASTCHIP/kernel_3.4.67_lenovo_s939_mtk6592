...
typedef struct
{
    u16 pid;                 //product id   //
    u16 vid;                 //version id   //
    u16 mid;                //lenovo-sw xuwen1 add 20121225 for read TP version
} st_tpd_info;
#pragma pack()
...
{
	GTP_ERROR("[ahe]:Adsdsfsfsafdsadsdasad \n");
    if ((!size) && (!id))
    {
...
/*lenovo-sw xuwen1 add 20140123 for area touch begin*/
    ...
    printk("[ahe]:ABS_MT_POSITION_Y_W %d \n",size);
    GTP_ERROR("ahe ahe ahe %d \n",size);
    #endif
/*lenovo-sw xuwen1 add 20140123 for area touch end*/
