...
int gain_flag = 0; //grace modify in 2014.3.12
u16 prev_als_value = 0; //grace modify in 2014.3.12
/*--------------------------------------------------------------------------
--*/
typedef struct {
	...
	if(enable)
	{
		...
		if(0 == obj->hw->polling_mode_ps)
		{
		        if(1 == ps_cali.valid)
			{
				...
				if(res <= 0)
				{
					APS_LOG("rpr410_enable_ps step 4 ret=%d\n", res);
					goto EXIT_ERR;
					return RPR410_ERR_I2C;
				}
				databuf[0] = REG_PSTL_MBS;	
				...
				if(res <= 0)
				{
					APS_LOG("rpr410_enable_ps step 5 ret=%d\n", res);
					goto EXIT_ERR;
					return RPR410_ERR_I2C;
				}
				databuf[0] = REG_PSTH_LSB;	
			...
			else
			{
				...
				if(res <= 0)
				{
					APS_LOG("rpr410_enable_ps step 7 ret=%d\n", res);
					goto EXIT_ERR;
					return RPR410_ERR_I2C;
				}
				databuf[0] = REG_PSTL_MBS;	
				...
				if(res <= 0)
				{
					APS_LOG("rpr410_enable_ps step 8 ret=%d\n", res);
					goto EXIT_ERR;
					return RPR410_ERR_I2C;
				}
				databuf[0] = REG_PSTH_LSB;	
				...
				if(res <= 0)
				{
					APS_LOG("rpr410_enable_ps step 9 ret=%d\n", res);
					goto EXIT_ERR;
					return RPR410_ERR_I2C;
				}
				databuf[0] = REG_PSTH_MBS; 
				...
				if(res <= 0)
				{
					APS_LOG("rpr410_enable_ps step 10 ret=%d\n", res);
					goto EXIT_ERR;
					return RPR410_ERR_I2C;
				}
			...
			if(res <= 0)
			{
				APS_LOG("rpr410_enable_ps step 11 ret=%d\n", res);
				goto EXIT_ERR;
				return RPR410_ERR_I2C;
			}
			...
			if(res <= 0)
			{
				APS_LOG("rpr410_enable_ps step 12 ret=%d\n", res);
				goto EXIT_ERR;
				return RPR410_ERR_I2C;
			}
			/*debug code for reading register value*/
...
static long rpr410_enable_ps_for_cali(struct i2c_client *client, int enable)
{
	struct rpr410_priv *obj = i2c_get_clientdata(client);
	u8 databuf[2];    
	int res = 0;
...
/*----------------------------------------------------------------------------*/
#if 1
static int rpr410_enable(struct i2c_client *client, int enable)
{
	struct rpr410_priv *obj = i2c_get_clientdata(client);
	u8 databuf[2];    
	int res = 0;
...
#if 1
static int rpr410_init_client_for_cali(struct i2c_client *client, u8 pulse)
{
...
#define JUDGE_FIXED_COEF (10)
#define MAX_OUTRANGE     (106822)
#define MAXRANGE_NMODE   (0xFFFF)
#define MAXSET_CASE      (7)
#define CUT_UNIT          2
	...
	else 
	{
		...
		else if (calc_judge < (data.als_data0 * judge_coefficient[4])) 
		{
			 set_case = 4;
		} 
		else if (calc_judge < (data.als_data0 * judge_coefficient[5])) 
		{
			 set_case = 5;
		} 
		else if (calc_judge < (data.als_data0 * judge_coefficient[6])) 
		{
			 set_case = 6;
		} 
		else
...
#define GAIN_VAL_MASK (0xF)
  ...
  rpr410_als_gain_val  = buffer[0];
  printk("rpr0410 ALS gain =%xh\n",buffer[0]);
  //grace modify in 2014.3.11 end
  return (0);
		
EXIT_ERR:
	APS_ERR("rpr410_get_from_device fail\n");
	return res;
...
int rpr410_read_als(struct i2c_client *client, u16 *data)
{
	struct rpr410_priv *obj = i2c_get_clientdata(client);	 
	u8 buffer[2];
	//u16 prev_als_value = *data; //grace modify in 2014.3.12
	int res = 0;
	READ_DATA_BUF   als_data;
	DEVICE_VAL  dev_val;
	u8 databuf[2];//grace modify in 2014.3.11
	...
	//grace modify in 2014.3.12 begin
	if(gain_flag == 1)
	{
		*data = prev_als_value;
		gain_flag = 0;
...
//get adc channel 1 value
	...
	if(res <= 0)
	{
        ...
        rpr410_chn0_val = als_data.als_data0 ;
       	rpr410_chn1_val = als_data.als_data1 ;
	...
	*data = calc_rohm_als_data(als_data, dev_val);
	...
	if(*data == CALC_ERROR)
		*data = prev_als_value;	//Report same value as previous.
		...
		prev_als_value = *data;  //grace modify in 2014.3.12

        printk("rpr0410 c0_value=%d, c1_value=%d, ALS_data=%d\n", als_data.als_data0, als_data.als_data1, *data);

	//grace modify in 2014.3.11 begin
	 if (als_data.als_data0<900)
	 {
	 		gain_flag = 1; //grace modify in 2014.3.12
	 		databuf[0] = REG_ALSPSCONTROL;
			databuf[1] = 0x2a;
			res = chip_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
			if(res <= 0)
	 ...
	 else if(als_data.als_data0>60000)
	 {
	 		gain_flag = 1; //grace modify in 2014.3.12
	 		databuf[0] = REG_ALSPSCONTROL;
			databuf[1] = 0x2;
...
/*----------------------------------------------------------------------------*/
/*for interrup work mode support -- by liaoxl.lenovo 12.08.2011*/
static void rpr410_ps_change_thd(struct i2c_client *client, int ps)
{
        struct rpr410_priv *obj = i2c_get_clientdata(client);
	...
	switch(ps)
	{
		case 0:
			...
			{
				lowt[0] = (u8)(ps_cali.far_away & 0x00FF);
				lowt[1] = (u8)((ps_cali.far_away & 0xFF00) >> 8);
				hight[0] = 0xFF;
				hight[1] = 0xFF;
			}
			chg = 1;
		...
		case 1:
			...
			else
			{
				lowt[0] = 0;
				lowt[1] = 0;
				hight[0] = (u8)(ps_cali.close & 0x00FF);
				hight[1] = (u8)((ps_cali.close & 0xFF00) >> 8);
			}
			chg = 1;
			break;
...
static void rpr410_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		if(0 == err) 
		{
			...
			//APS_LOG("rpr410_get_ps_value  ps =%d!\n", temp);
		        if(temp == 0)
			{
				...
				//APS_LOG("rpr410_eint_work	ps close!\n");
				if((err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data)))
				{
				     APS_ERR("call hwmsen_get_interrupt_data fail = %d\n", err);
				}
                                /*lenovo-sh chenlj2 2013-7-23 add for singal interrupt function add*/
				rpr410_ps_change_thd(obj->client, 0);
			}
			else if(temp == 1)
			{
				...
				//let up layer to know
				//APS_LOG("rpr410_eint_work	ps far away!\n");
				if((err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data)))
				{
					APS_ERR("call hwmsen_get_interrupt_data fail = %d\n", err);
				}
                                /*lenovo-sh chenlj2 2013-7-23 add for singal interrupt function add*/
				rpr410_ps_change_thd(obj->client, 1);
			}
		}
	}
...
static void rpr410_WriteCalibration(HWMON_PS_STRUCT *data_cali)
{
    struct PS_CALI_DATA_STRUCT *ps_data_cali;
	APS_ERR("rpr410_WriteCalibration  1 close=%d,far_away=%d\n" ,data_cali->close,data_cali->far_away);
	APS_LOG("rpr410_WriteCalibration  2 far_away =%d,\n" ,data_cali->far_away);
	APS_LOG("rpr410_WriteCalibration  3 valid =%d,\n", data_cali->valid);
	//APS_LOG("rpr410_WriteCalibration  4 %d,", data_cali->pulse);
	...
	if(data_cali->valid == 1)
	{
		...
		atomic_set(&rpr410_obj->ps_thd_val_high, ps_data_cali->close);
	        atomic_set(&rpr410_obj->ps_thd_val_low,  ps_data_cali->far_away);
	}

}
/*lenovo-sh chenlj2 2013-7-23 modified close & far thread start*/
//#define THRES_TOLERANCE		26	//I think this is a proper value. It should not be too big.
//#define THRES_DEFAULT_DIFF	10 //15 //35
/*lenovo-sh chenlj2 2013-7-23 modified close & far thread end*/
extern int RPR410_THRES_TOLERANCE;
extern int RPR410_THRES_DEFAULT_DIFF;
#define REG_PSTH_MAX	0xFFF
#define REG_PSTL_MAX	0xFFF

static int rpr410_ps_average_val = 0; 
static int rpr410_read_data_for_cali(struct i2c_client *client, HWMON_PS_STRUCT *ps_data_cali)
{
	struct rpr410_priv *obj = i2c_get_clientdata(client);
        u8 databuf[2];
        u8 buffer[2];  
	...
	{
		mdelay(5);//50
		res = rpr410_read_ps(client,&data[i]);
		if(res != 0)
		{
			APS_ERR("rpr410_read_data_for_cali fail: %d\n", i); 
			break;
		}
		else
		{
	...
	{
		data_cali = sum / 20;
		rpr410_ps_average_val = data_cali;
		APS_LOG("rpr410_read_data_for_cali data = %d",data_cali);
		//lenovo-sw, shanghai, add by chenlj2,  2013-07-23 begin
		#if 1
		 if( data_cali>600)
		{
			APS_ERR("rpr410_read_data_for_cali fail value to high: %d\n", data_cali);
			return -2;
		}
		#endif
		//lenovo-sw, shanghai, add by chenlj2, 2013-07-23 end

		ps_data_cali->close =data_cali + RPR410_THRES_TOLERANCE + RPR410_THRES_DEFAULT_DIFF;
		ps_data_cali->far_away =data_cali + RPR410_THRES_TOLERANCE;
		
		ps_data_cali->valid = 1;
		APS_LOG("rpr410_read_data_for_cali close = %d,far_away = %d,valid = %d\n",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid);
		//APS_LOG("rpr410_read_data_for_cali avg=%d max=%d min=%d\n",data_cali, max, min);
	}
       ...
       unsigned long arg)
{
	...
	switch (cmd)
	{
		case ALSPS_SET_PS_MODE:
			...
			if(enable)
			{
				if(err = rpr410_enable_ps_for_cali(obj->client, 1))
				{
			...
			{
				if(err = rpr410_enable_ps_for_cali(obj->client, 0))
				{
		...
		case ALSPS_SET_PS_CALI:
			...
			rpr410_WriteCalibration(&ps_cali_temp);
			APS_ERR(" ALSPS_SET_PS_CALI %d,%d,%d\t",ps_cali_temp.close,ps_cali_temp.far_away,ps_cali_temp.valid);
			break;
		...
		case ALSPS_GET_PS_RAW_DATA_FOR_CALI:
		{
				...
				rpr410_init_client_for_cali(obj->client, 0);
				msleep(200);
				err = rpr410_read_data_for_cali(obj->client,&ps_cali_temp);
				if(err < 0 ){
					goto err_out;
				}	
