...
const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	/*{//CALIBRATION_LAYOUT_SLIM_LSC1 without Defect //data sheet of excel : "Slim"
		0x00000, 0x00000001, CAM_CAL_SINGLE_OTP_DATA,
		{		
			...
			{0x00000001, 0x00000015, 0x0000000F, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
...
static int GetAWBGain(u16* rGain, u16* gGain, u16* bGain,u16* rGoldenGain, u16* gGoldenGain, u16* bGoldenGain, u8* otpData, u16 bufSize)
{
	u16 i;
	u32 checksum = 0;
	...
	//2.get awb data
	*rGain = (*(otpData)<<8)|(*(otpData+1));
	*bGain = (*(otpData+2)<<8)|(*(otpData+3));
	*gGain = (*(otpData+4)<<8)|(*(otpData+5));

	*rGoldenGain = (*(otpData+6)<<8)|(*(otpData+7));
	*bGoldenGain = (*(otpData+8)<<8)|(*(otpData+9));
	*gGoldenGain = (*(otpData+10)<<8)|(*(otpData+11));	
	CAM_CAL_LOG_IF(dumpEnable,"rGain = %d, bGain = %d, gGain = %d\n", *rGain, *bGain, *gGain);
	return 1;
}

static int GetAFGain(u16* AFInf, u16* AFMacro, u8* otpData, u16 bufSize)
{
	CAM_CAL_LOG_IF(dumpEnable,"GetAF ENTER:\n");

	//2.get af data
	*AFInf = (*(otpData)<<8)|(*(otpData+1));
	*AFMacro = (*(otpData+2)<<8)|(*(otpData+3));
...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
	...
	u16 rGain, gGain, bGain,rGoldenGain, gGoldenGain, bGoldenGain;
    u8 awbBuf[12];
	UINT8 AWBAFVersion,AWBAFVersionAddr;
    u8 afBuf[4];
    int ret;
	//Structure
	/*
	Byte[0]:module
	Byte[1]: lens
	Byte[32:20]: {GoldenB, GoldenGb, GoldenGr, GoldenR, UnitB, UnitGb, UnitGr, UnitR}
	Byte[34:33]: AF inf.
	Byte[36:35]: AF Macro
	*/
    if(pCamCalData->DataVer >= CAM_CAL_N3D_DATA)
    ...
    { 
        ...
        else
        {
			...
			cam_calCfg.u4Offset = 0x14; //awb and af flag
			...
			pCamCalData->Single2A.S2aVer = 0x01;
            pCamCalData->Single2A.S2aBitEn = (0x0F & AWBAFConfig);
			CAM_CAL_LOG_IF(dumpEnable,"S2aBitEn=0x%x", pCamCalData->Single2A.S2aBitEn);
            pCamCalData->Single2A.S2aAfBitflagEn = (0x03 & AWBAFConfig);// //Bit: step 0(inf.), 1(marco), 2, 3, 4,5,6,7
            //memset(pCamCalData->Single2A.S2aAf,0x0,sizeof(pCamCalData->Single2A.S2aAf));
			CAM_CAL_LOG_IF(dumpEnable,"AWBAFConfig=0x%x", AWBAFConfig);
            if(0x1&AWBAFConfig){
                ////AWB////
				CAM_CAL_LOG_IF(dumpEnable,"start_addr=0x%x", start_addr);
                //start_addr = 0x15;
            	ret = CamCalReadOTP(CamcamFID, start_addr, &awbBuf[0], 12);


                ret = GetAWBGain(&rGain, &gGain, &bGain,&rGoldenGain, &gGoldenGain, &bGoldenGain, &awbBuf[0], 12);
				CAM_CAL_LOG_IF(dumpEnable,"Read CalGain OK\n");
    	        CAM_CAL_LOG_IF(dumpEnable,"Start assign value\n");
				#if 0
				AWBAFVersionAddr = 0x13; 
            	ret = CamCalReadOTP(CamcamFID, AWBAFVersionAddr, &AWBAFVersion, 1);
				CAM_CAL_LOG_IF(dumpEnable,"AWBAFVersion=0x%x", AWBAFVersion);
				if(AWBAFVersion == 0x01)
				{
					//gGain =1/gGain;
	                pCamCalData->Single2A.S2aAwb.rUnitGainu4R = (u32) ((1024.0/bGain)*512);
	                pCamCalData->Single2A.S2aAwb.rUnitGainu4G = (u32) ((gGain/1024.0)*512);
	                pCamCalData->Single2A.S2aAwb.rUnitGainu4B = (u32) ((1024.0/rGain)*512);

	                pCamCalData->Single2A.S2aAwb.rGoldGainu4R = (u32) ((1024.0/bGoldenGain)*512);
	                pCamCalData->Single2A.S2aAwb.rGoldGainu4G = (u32) ((gGoldenGain/1024.0)*512);
	                pCamCalData->Single2A.S2aAwb.rGoldGainu4B = (u32) ((1024.0/rGoldenGain)*512);
				}
				else
				{
	                ...
	                pCamCalData->Single2A.S2aAwb.rGoldGainu4R = (u32) ((1024.0/rGoldenGain)*512);
	                pCamCalData->Single2A.S2aAwb.rGoldGainu4G = (u32) ((1024.0/gGoldenGain)*512);
	                pCamCalData->Single2A.S2aAwb.rGoldGainu4B = (u32) ((1024.0/bGoldenGain)*512);
				}
    			#else
	                pCamCalData->Single2A.S2aAwb.rUnitGainu4R = (u32) ((1024.0/rGain)*512);
	                pCamCalData->Single2A.S2aAwb.rUnitGainu4G = (u32) ((1024.0/gGain)*512);
			...
			if(0x2&AWBAFConfig){
                ////AF////
                ret = CamCalReadOTP(CamcamFID, start_addr+12, &afBuf[0], 4);
                ret = GetAFGain(&AFInf, &AFMacro,  &afBuf[0], 4);
...
UINT32 DoCamCalLayoutCheck(void)
{
    ...
    if ((CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0]) == 0))
    {
        ...
        {
			CAM_CAL_LOG_IF(dumpEnable,"cam_calCfg.u4Offset= 0x%x,CalLayoutTbl[i].HeaderAddr=%x", cam_calCfg.u4Offset,CalLayoutTbl[i].HeaderAddr);
			...
			CAM_CAL_LOG_IF(dumpEnable,"CalLayoutTbl[%d].HeaderId= 0x%x",i, CalLayoutTbl[i].HeaderId);
