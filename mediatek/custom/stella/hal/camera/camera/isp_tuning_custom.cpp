...
)   const
{
    ...
    MINT32 i4DaylightFluorescentIndex = rCamInfo.rAWBInfo.i4DaylightFluorescentIndex;
...
//                   THH2  TH2  THL2                   THH1  TH1  THL1
   ...
   LOGD(
       " i4DaylightFluorescentIndex = %d, CCT=(%d,%d)\n"
       ,i4DaylightFluorescentIndex, rCamInfo.eIdx_Shading_CCT,eIdx_Shading_CCT_new
   );
   LOGD(
       " i4SceneLV = %d, CCT=%d\n"
       ,i4SceneLV, rCamInfo.eIdx_Shading_CCT
   );

if(i4SensorID == 0x8825){
    // case for DF
    if (eIdx_Shading_CCT_new == eIDX_Shading_CCT_D65){
	if (i4DaylightFluorescentIndex < DAYLIGHT_F_IDX){
           LOGD(
               " Switch to i4DaylightFluorescentIndex = %d\n"
    ...
    if ((rCamInfo.eIdx_Shading_CCT == eIDX_Shading_CCT_D65) && (eIdx_Shading_CCT_new == eIDX_Shading_CCT_CWF)){
        if ((i4CCT >= 4500) && (i4CCT < 5000)){
            if (i4SceneLV >= LV_L){
               LOGD(
                   " Switch to i4SceneLV(L) = %d\n"
                   ,i4SceneLV
               );
                 eIdx_Shading_CCT_new = eIDX_Shading_CCT_D65; 
            }
