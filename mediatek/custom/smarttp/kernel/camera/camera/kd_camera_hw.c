...
#endif
#define	USE_VBAT_LDO
#define	USE_VBAT_BEFORE_LDO
#define  	USE_PMU_BEFORE
//#define 	USE_PMU_AFTER
...
u32 pinSet[2][8] = {
    ...
    if (On) {
		...
		{
			printk("kdCISModulePowerOn get in--%s\n",currSensorName);
			...
			if(0 == strcmp(SENSOR_DRVNAME_OV12830_MIPI_RAW,currSensorName)){
			...
			if(0==strcmp(SENSOR_DRVNAME_OV5693_MIPI_RAW,currSensorName)){
				if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
				{
					PK_DBG("[CAMERA SENSOR] Fail to enable analog power\n");
					//return -EIO;
			...
			//main camera DVDD  VGP1_PMU
			if(0 == strcmp(SENSOR_DRVNAME_OV12830_MIPI_RAW,currSensorName))
			{

				#ifdef USE_PMU_AFTER
				if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D_MAIN, VOL_1300,mode_name))
				{
					 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
					 //return -EIO;
					 goto _kdCISModulePowerOn_exit_;
				...
				#endif
				#ifdef USE_VBAT_LDO
				printk("Set MAIN CAMERA_POWER_PULL_PIN for power \n");
			...
			//sub camera DVDD VGP5_PMU
			if(0==strcmp(SENSOR_DRVNAME_OV5693_MIPI_RAW,currSensorName))
			{
				#ifdef USE_PMU_BEFORE
				...
				#endif
				#ifdef USE_VBAT_BEFORE_LDO
				printk("Set SUB CAMERA_POWER_PULL_PIN for power \n");
				if (mt_set_gpio_pull_enable(GPIO_CAMERA_BEFORE_LDO_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[[CAMERA SENSOR] Set CAMERA_POWER_PULL_PIN DISABLE ! \n"); }
				if(mt_set_gpio_mode(GPIO_CAMERA_BEFORE_LDO_EN_PIN, GPIO_CAMERA_BEFORE_LDO_EN_PIN_M_GPIO)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN mode failed!! \n");}
				if(mt_set_gpio_dir(GPIO_CAMERA_BEFORE_LDO_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN dir failed!! \n");}
				if(mt_set_gpio_out(GPIO_CAMERA_BEFORE_LDO_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[[CAMERA SENSOR] set CAMERA_POWER_PULL_PIN failed!! \n");}
				#endif
  ...
  //lenovo.sw START 20131029 wangsx3 change camera timing-----------------------------------------------
		else if ((currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825SUNNY_MIPI_RAW,currSensorName)))
            			||(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825QTECH_MIPI_RAW,currSensorName))))
		{
			    //PK_DBG("kdCISModulePowerOn get in---  SENSOR_DRVNAME_OV8825_MIPI_RAW \n");
			    //PK_DBG("[ON_general 2.8V]sensorIdx:%d \n",SensorIdx);
			    //1st:DOVDD,,MT6323_POWER_LDO_VCAM_IO
			    if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
			    {
			      PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
			      //return -EIO;
  ...
  //lenovo.sw START 20131031 wangsx3 change SUB camera timing-----------------------------------------------//
  		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV9760_MIPI_RAW,currSensorName)))
		  {
		    ...
		    //1:DOVDD,,MT6323_POWER_LDO_VCAM_IO
		    if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
		    {
		      PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
		      //return -EIO;
  ...
  //lenovo.sw END 20131031 wangsx3 change SUB camera timing-----------------------------------------------//	
    ...
    else
    {
		...
		//lenovo.sw 20131101 wangsx3 fix timnig issue
		if(currSensorName && ((0 == strcmp(SENSOR_DRVNAME_OV12830_MIPI_RAW,currSensorName))||
								(0==strcmp(SENSOR_DRVNAME_OV5693_MIPI_RAW,currSensorName))))
		{
			printk("kdCISModulePower--off get in--%s \n",currSensorName);
			...
			if(0 == strcmp(SENSOR_DRVNAME_OV12830_MIPI_RAW,currSensorName))
			{
				...
				#endif
				#ifdef USE_VBAT_LDO
				printk("Set MAIN GPIO118 for power OFF\n");
			...
			if(0==strcmp(SENSOR_DRVNAME_OV5693_MIPI_RAW,currSensorName))
			{
				...
				#endif
				#ifdef USE_VBAT_BEFORE_LDO
				printk("Set SUB GPIO117 for power OFF\n");
				if (mt_set_gpio_pull_enable(GPIO_CAMERA_BEFORE_LDO_EN_PIN, GPIO_PULL_DISABLE)) {PK_DBG("[CAMERA SENSOR] Set GPIO94 PULL DISABLE ! \n"); }
				if(mt_set_gpio_mode(GPIO_CAMERA_BEFORE_LDO_EN_PIN, GPIO_CAMERA_BEFORE_LDO_EN_PIN_M_GPIO)){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
				if(mt_set_gpio_dir(GPIO_CAMERA_BEFORE_LDO_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
				if(mt_set_gpio_out(GPIO_CAMERA_BEFORE_LDO_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
				#endif
			}
		...
		if((currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825_MIPI_RAW,currSensorName)))
                 ||(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825SUNNY_MIPI_RAW,currSensorName)))
                 ||(currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV8825QTECH_MIPI_RAW,currSensorName))))
		{
