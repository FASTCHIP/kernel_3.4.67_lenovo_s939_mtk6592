...
#define OV12830_BG_Ratio_Typical 0x143
#define OV12830_RG_Ratio_Typical 0x142
struct OV12830_otp_struct {                                         
...
int OV12830_check_otp_wb(int index)                                 
{                                                           
	int flag, i;                                     
	int bank, address;                               
	// select bank index                             
...
//                      1, group index has invalid data                    
//                      2, group index has valid data                      
int OV12830_check_otp_lenc(int index)                                              
{                                                                          
	int flag, i, bank;                                              
	int address;                                                    
	// select bank: 4, 8, 12                                        
...
// index: index of otp group. (1, 2, 3)                                    
// otp_ptr: pointer of OV12830_otp_struct                                          
// return:              0,                                                 
int OV12830_read_otp_wb(int index, struct OV12830_otp_struct *otp_ptr)                     
{                                                                          
	int i, bank;                                                    
	int address;                                                    
	int temp;                                                       
...
// otp_ptr: pointer of OV12830_otp_struct                                                         
// return:            0,                                                                  
int OV12830_read_otp_lenc(int index, struct OV12830_otp_struct *otp_ptr)                                  
{                                                                                         
	int bank, i;                                                                   
	int address;                                                                   
	// clear otp buffer                                                            
...
// return 0;                                                                       
int OV12830_update_awb_gain(int R_gain, int G_gain, int B_gain)                            
{   
...
// otp_ptr: pointer of OV12830_otp_struct                                                  
int OV12830_update_lenc(struct OV12830_otp_struct *otp_ptr)                                        
{                                                                                  
	int i, temp;                                                             
...
//                     1, no OTP                                                        
int OV12830_update_otp_wb()                                                                     
{                                                                                       
	struct OV12830_otp_struct current_otp;                                              
	int i;                                                                      
	int otp_index;                                                              
	int temp;                                                                   
	...
	OV12830_read_otp_wb(otp_index, &current_otp);                                       
	...
	//calculate G gain                                                          
	//0x400 = 1x gain                                                           
	if(bg < OV12830_BG_Ratio_Typical) {                                                 
		if (rg< OV12830_RG_Ratio_Typical) {
			// current_otp.bg_ratio < OV12830_BG_Ratio_Typical &&                                          
			// current_otp.rg_ratio < OV12830_RG_Ratio_Typical       
			G_gain = 0x400;                                  
			B_gain = 0x400 * OV12830_BG_Ratio_Typical / bg;          
			R_gain = 0x400 * OV12830_RG_Ratio_Typical / rg;                     
		}else{                                                      
			         // current_otp.bg_ratio < OV12830_BG_Ratio_Typical &&    
			         // current_otp.rg_ratio >= OV12830_RG_Ratio_Typical      
			R_gain = 0x400;                                             
			         G_gain = 0x400 * rg / OV12830_RG_Ratio_Typical;          
			B_gain = G_gain * OV12830_BG_Ratio_Typical /bg;                     
	   }                                                           
	}                                                                     
	else 
	{                                                                
		if (rg < OV12830_RG_Ratio_Typical) {                                
		         ...
		         // current_otp.rg_ratio < OV12830_RG_Ratio_Typical       
		B_gain = 0x400;                                             
		G_gain = 0x400 * bg / OV12830_BG_Ratio_Typical;                     
		R_gain = G_gain * OV12830_RG_Ratio_Typical / rg;                    
		}                                                           
		else {                                                      
		         // current_otp.bg_ratio >= OV12830_BG_Ratio_Typical &&   
		         // current_otp.rg_ratio >= OV12830_RG_Ratio_Typical      
		G_gain_B = 0x400 * bg / OV12830_BG_Ratio_Typical;                   
		         G_gain_R = 0x400 * rg / OV12830_RG_Ratio_Typical;        
       ...
       OV12830DB("OTP_TEST R_G=0x%04x,G_G=0x%04x,B_G=0x%04x\r\n",B_gain,G_gain,B_gain);
	OV12830_update_awb_gain(R_gain, G_gain, B_gain);                              
	return 0;                                                             
}                                                                                 
// call this function after OV12830 initialization                                
// return value:      0 update success                                            
//                    1, no OTP                                                   
int OV12830_update_otp_lenc()                                                             
{                                                                                 
	struct OV12830_otp_struct current_otp; 
	int i;                                                                                        
	int otp_index;                                         
	...
	OV12830_read_otp_lenc(otp_index, &current_otp);                
	OV12830_update_lenc(&current_otp);                             
	// success                                             
	return 0;                                              
}                                                                  
// call this function after OV12830 initialization                 
...
OV12830_write_cmos_sensor(0x4837,0x0b);
OV12830_write_cmos_sensor(0x5000,0x86);
OV12830_write_cmos_sensor(0x5002,0x00);
...
OV12830_write_cmos_sensor(0x5000,0x86);
OV12830_write_cmos_sensor(0x5002,0x00);
OV12830_write_cmos_sensor(0x5003,0x01);
OV12830_write_cmos_sensor(0x0100,0x01);
#else
OV12830_write_cmos_sensor(0x0100,0x00);
OV12830_write_cmos_sensor(0x30b3,0x54);
...
static void OV12830_Sensor_Init(void)
{
	...
	kfree(pBuf);
	OV12830_update_otp_wb();
	OV12830_update_otp_lenc();
#endif	
...
{
    kal_int16 mirror=0,flip=0;
    flip = OV12830_read_cmos_sensor(0x3820);
    mirror= OV12830_read_cmos_sensor(0x3821);
 
    switch (imgMirror)
    {
        case IMAGE_V_MIRROR://IMAGE_H_Mirror:
            OV12830_write_cmos_sensor(0x3820, (flip & (0xBD))); //Set normal
            OV12830_write_cmos_sensor(0x3821, (mirror & (0xF9)));//Set Mirror
            break;
        case IMAGE_HV_MIRROR://IMAGE_Normal:
            OV12830_write_cmos_sensor(0x3820, (flip & (0xBD))); //Set Normal
            OV12830_write_cmos_sensor(0x3821, (mirror | (0x06))); //Set Normal
            break;
        case IMAGE_NORMAL://IMAGE_HV_MIRROR:
            OV12830_write_cmos_sensor(0x3820, (flip |(0x42))); //Set flip
            OV12830_write_cmos_sensor(0x3821, (mirror & (0xF9)));//Set mirror
            break;
        case IMAGE_H_MIRROR://IMAGE_V_MIRROR:
            OV12830_write_cmos_sensor(0x3820, (flip |(0x42))); //Set flip
            OV12830_write_cmos_sensor(0x3821, (mirror |(0x06)));//Set Normal
            break;
    } 
